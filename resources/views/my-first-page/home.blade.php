<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
</head>
<body>
    <h1>My first page Home</h1>
    <h2>Welcome {{ $user }}</h2>
    <nav>
        <ul>
            <li><a href="{{ route('my-first-page.home') }}">Home</a></li>
            <li><a href="{{ route('my-first-page.about-us') }}">About Us</a></li>
            <li><a href="{{ route('my-first-page.contact') }}">Contact</a></li>
        </ul>
    </nav>
    <h4>Hora actual: {{ $actualTime }}</h4>
    @if($user === 'Usuario')
        <h3>Personaliza la pagina</h3>
        <form action="{{ route('my-first-page.personalize') }}" method="post">
            @csrf
            @method('POST')
            <label for="name">Nombre</label>
            <input type="text" name="name" value="{{ $user }}" placeholder="Introduce tu nombre...">
            <input type="submit" value="Personalizar">
        </form>
    @endif
</body>
</html>
